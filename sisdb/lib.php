...
function cegep_sisdb_sync($start_term) {
    ...
    while ($sisdbsource_rs && !$sisdbsource_rs->EOF) {
        ...
        if (!in_array($student, $students)) {
            ...
            if ($result && $result->RecordCount() == 0) {
                ...
                if (!$result) {
                    trigger_error($sisdb->ErrorMsg() .' STATEMENT: '. $insert, E_USER_WARNING);
            ...
            elseif ($result && ($result->fields['lastname'] != $student_lastname || $result->fields['firstname'] != $student_firstname || $result->fields['program_id'] != $program || $result->fields['program_year'] != $programyear)) {
                ...
                if (!$result) {
                    trigger_error($sisdb->ErrorMsg() .' STATEMENT: '. $update, E_USER_WARNING);
                    echo "Sync error : student process";
                    break;
        ...
        if (!in_array($program, $programs)) {
            ...
            if ($result && $result->RecordCount() == 0) {
                ...
                if (!$result) {
                    trigger_error($sisdb->ErrorMsg() .' STATEMENT: '. $insert, E_USER_WARNING);
                    echo "Sync error : program process";
                    break;
    ...
    foreach ($student_enrolments_del as $enrolment) {
        ...
        if (!$result = $sisdb->Execute($delete)) {
            trigger_error($sisdb->ErrorMsg() .' STATEMENT: '. $delete, E_USER_WARNING);
            echo "Erreur : inscription process";
            break;
        }
    ...
    if (CLI_SCRIPT) {
        // Display plaintext report
        $msg = "Synchronization completed\n";
        $msg .= sprintf("Terms : %s\n", implode(', ', $terms));
        $msg .= sprintf("Students : %d added; %d updated; %d processed\n", $count['students_added'], $count['students_updated'], count($students));
        $msg .= sprintf("Programs : %d added; %d updated; %d processed\n", $count['programs_added'], $count['programs_updated'], count($programs));
        $msg .= sprintf("Courses : %d added; %d updated; %d processed\n", $count['courses_added'], $count['courses_updated'], count($courses));
        $msg .= sprintf("Coursegroups : %d added; %d processed\n", $count['coursegroups_added'], count($coursegroups));
        $msg .= sprintf("Student course enrolments : %d added; %d removed; %d skipped; %d processed\n", $count['student_enrolments_added'], $count['student_enrolments_removed'], $count['records_skipped'], count($student_enrol_remotedb));
        $msg .= sprintf("Student program enrolments : %d added; %d removed\n", $count['student_program_enrolments_added'], $count['student_program_enrolments_removed']);
        $msg .= sprintf("Teacher course enrolments : %d added; %d removed; %d processed\n", $count['teacher_enrolments_added'], $count['teacher_enrolments_removed'], count($teacher_enrol_remotedb));

        $msg .= "Started at : $mysqltimestamp\n";
    ...
    if (!empty($latest_term)) {
        ...
        while ($coursegroups_rs && !$coursegroups_rs->EOF) {
            ...
            if (!$result = $sisdb->Execute($delete)) {
                ...
                echo "Error pruning old student enrolment records.";
            } else {
            ...
            if (!$result = $sisdb->Execute($delete)) {
                trigger_error($sisdb->ErrorMsg() .' STATEMENT: '. $delete);
                echo "Error pruning old teacher enrolment records.";
            } else {
            ...
            if (!$result = $sisdb->Execute($delete)) {
                trigger_error($sisdb->ErrorMsg() .' STATEMENT: '. $delete);
                echo "Error pruning old coursegroup records.";
            } else {
        ...
        if (!$result = $sisdb->Execute($delete)) {
            trigger_error($sisdb->ErrorMsg() .' STATEMENT: '. $delete);
            echo "Error pruning old course records.";
        } else {
        ...
        if (!$result = $sisdb->Execute($delete)) {
            trigger_error($sisdb->ErrorMsg() .' STATEMENT: '. $delete);
            echo "Error pruning old student records.";$coursegroup_id = $coursegroups->fields['coursegroup_id'];
        } else {
        ...
        if (!$result = $sisdb->Execute($delete)) {
            trigger_error($sisdb->ErrorMsg() .' STATEMENT: '. $delete);
            echo "Error pruning old program records.";
        } else {
...
function cegep_sisdb_close() {
    global $enroldb, $sisdb, $sisdbsource;
